...
library(kableExtra)
=======
install.packages("readxl") install.packages("dplyr") install.packages("ggplot2") install.packages("tidyr") install.packages("kableExtra") install.packages("openxlsx") install.packages(c("stringr", "scales", "plotly", "ggrepel", "ggthemes"))

library(readxl) library(openxlsx) library(dplyr) library(ggplot2) library(tidyr) library(stringr) library(scales) library(plotly) library(ggrepel) library(ggthemes) library(kableExtra)
>>>>>>> Stashed changes
...
ggplot(Esler_arasi_yas_farki, aes(x = "", y = deger, fill = kategori)) +
  geom_bar(stat = "identity", width = 1) +
  coord_polar("y", start = 0) +
  labs(title = "Yaş Farki Dagilimi (Turkiye Geneli)") +
  theme_void()+theme(
    plot.title = element_text(size = 14, face = "bold", hjust = 0.5),  
    legend.title = element_text(size = 12),  
    legend.text = element_text(size = 10)  
  ) +
  scale_fill_manual(values = c("skyblue", "red", "darkgreen", "purple"))
...
# 4. Grafik çizimi
colors <- c("deepskyblue", "darkorange", "forestgreen", "steelblue")
radarchart(data,
           axistype = 1,
           pcol = colors,
       ...
       col = colors, lwd = 3, bty = "n", horiz = TRUE, cex = 0.8)
...
str(veri_long)
=======
turkiye_geneli \<- read_excel("veri/turkiye_geneli.xlsx") head(turkiye_geneli) Kadin_Erkek_Yas_Dagilimlari \<- read_excel("veri/Kadin_Erkek_Yas_Dagilimlari.xlsx") head(Kadin_Erkek_Yas_Dagilimlari) Evlilik_suresi_Turkiye_geneli \<- read_excel("veri/Evlilik_suresi_Turkiye_geneli.xlsx") head(Evlilik_suresi_Turkiye_geneli)
>>>>>>> Stashed changes
=======
>>>>>>> 14b8039f703c2d1e31a5566c5b765a82d6b532da
...
grafik <- turkiye_geneli |> ggplot() +
  geom_line(aes(x = yil, y = evlenme_salaryisi, color = "Evlenme Sayısı",group=1), size = 1.7) +
...
evlilik_bosanma_hiz <- evlilik_bosanma_hiz %>%
  ...
  ggplot(veri, aes(x = Yil)) +
  geom_bar(aes(y = Evlenme_Sayisi), stat = "identity", fill = "cornflowerblue") +
  ...
  geom_point(aes(y = Kaba_Bosanma_Hizi * 80000), color = "darkgreen", size = 2) +
