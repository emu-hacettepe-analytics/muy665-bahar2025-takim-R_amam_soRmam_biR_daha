...
```{r, fig.width=12, fig.height=10}
# Kadın-Erkek Yaşlarına GÖre Boşanma Sayının Pie Chart Üzerinden GÖsterimi
...
pie(kadin_veri, labels = kadin_etiketler, main = "Bosanma Yaş Dağılımı (Kadın)", col = renkler,font=2,cex=0.7)

pie(erkek_veri, labels = erkek_etiketler, main = "Bosanma Yaş Dağılımı (Erkek)", col = renkler,font=2,cex=0.7)
...
p6<-ggplot(df_erkek, aes(x = Yil, y = Bosanan_Erkek_Sayisi, color=Yas_Grubu , group = Yas_Grubu)) +
   geom_line(linewidth = 1.2) +
  geom_point(size = 2) +
  labs(
    title = "Yas Dagılımlarına Gore Bosanma Sayıları (Erkek)",
    x = "Yıl",
    y = "Bosanan Erkek Sayısı",
    color = "Yas Grubu"
  )  + theme_minimal() +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1,size=12),
...
# bosanma sayılarını yıl bazlı olarak toplanması 
...
ggplot(Esler_arasi_yas_farki, aes(x = kategori, y = deger, fill = kategori)) +
  ...
  labs(
    title = "Eşler Arası Yaş Farkına Gore Bosanma Sayıları (Turkiye Geneli)",
    x = " Eşler Arası Yaş Farkı",
    y = "Toplam Bosanma Sayısı",
    fill = "Evlilik Sureleri"
  ) +
  theme_minimal() +
  theme(
