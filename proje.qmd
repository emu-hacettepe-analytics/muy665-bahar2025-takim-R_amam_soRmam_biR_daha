...
>>>>>>> cf74cde081f95bb4cb1b3df11413d9a283b97f10
...
# Excel dosyasını oku (dosya adını kendi bilgisayar yoluna göre değiştir)
...
#Her yaş Grubuna Renk Belirleme
renkler <- c("Yaş Aralığı: 16-29"  = "skyblue",
  "Yaş Aralığı: 30-44"  = "salmon",
  "Yaş Aralığı: 45-54"  = "gold",
  "Yaş Aralığı: 55 ve 60 ustu"  = "red")
  
#Yaş grubuna denk gelen renkleri seçme
renk_atama <- renkler[yas_gruplari]

#Yüzdeleri hesapla (kadın ve erkek için ayrı)
kadin_yuzde <- round(kadin_veri / sum(kadin_veri) * 100, 1)
erkek_yuzde <- round(erkek_veri / sum(erkek_veri) * 100, 1)

# Etiketleri yüzdeyle birleştir (örnek: "Yaş Aralığı: 30-44 - 56.2%")
kadin_etiketler <- paste0(yas_gruplari, " - ", kadin_yuzde, "%")
erkek_etiketler <- paste0(yas_gruplari, " - ", erkek_yuzde, "%")

#iki grafiği beraber çizmek için
par(mfrow=c(1,2))

# Kadın pasta grafiği çizme
kadin_grafik <- pie(kadin_veri, labels = yas_gruplari, main = "Kadın Yaş Dağılımı", col = renk_atama)

# Erkek pasta grafiği çizme
erkek_grafik <- pie(erkek_veri, labels = yas_gruplari, main = "Erkek Yaş Dağılımı", col = renk_atama)
